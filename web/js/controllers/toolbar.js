    ...
    $("#toolbar [data-type]").click(function (event) {//给工具栏绑定事件
        ...
        switch (type) {
            ...
            case "horizon-space"://水平间距
                ...
                $selected.each(function () {//遍历
                    if (this !== $first.get(0) && this !== $last.get(0)) {//排除一个元素和最后一个元素
                        $(this).css({"left": position + space});//设置css
            ...
            case "vertical-space"://垂直间距
                ...
                $selected.each(function () {
                    if (this !== $first.get(0) && this !== $last.get(0)) {//排除最后一个元素和第一个元素
                        $(this).css({"top": position + space});//设置它们的top值
            ...
            case "put-bottom"://置低
                ...
                break;
            case "left"://移动到左边
                $selected.css({"left": 0, "right": "auto"});//设置样式
                break;
            case "center"://移动到中间
                $selected.each(function () {
                    var left = width / 2 - $(this).outerWidth() / 2;//工作区宽度除2-这个元素的外边距
                    $(this).css({"left": left, "right": "auto"});//设置样式
                });
                break;
            case "right"://移动到右边
                $selected.css({"left": "auto", "right": 0});//设置样式
                break;
            case "top"://移动到顶端
                $selected.css({"top": 0, "bottom": "auto"});//设置样式
                break;
            case "middle"://移动到中部
                $selected.each(function () {
                ...
                break;
            case "bottom"://移动到底部
                $selected.css({"top": "auto", "bottom": 0});//设置样式
                break;
